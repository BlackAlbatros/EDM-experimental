...
const androidRoot = path.join(process.cwd(), "android", "app", "src", "main", "res");
const outRoot = fs.existsSync(androidRoot) ? androidRoot : path.join(process.cwd(), "android-assets");
...
async function main() {
  const buffer = await fetch(SRC).then((r) => r.arrayBuffer()).then((b) => Buffer.from(b));
  ...
  for (const [bucket, size] of Object.entries(mipmap)) {
    ...
    await sharp(buffer).resize(size, size, { fit: "contain", background: { r: 0, g: 0, b: 0, alpha: 0 } }).png().toFile(file);
  ...
  for (const [bucket, size] of Object.entries(splashSize)) {
    ...
    const canvas = sharp({ create: { width: size, height: size, channels: 4, background: { r: 0, g: 0, b: 0, alpha: 1 } } });
    const logo = await sharp(buffer).resize(Math.floor(size * 0.5)).png().toBuffer();
    await canvas.composite([{ input: logo, gravity: "centre" }]).png().toFile(file);
  }

  console.log("Assets generated in:", outRoot);
  console.log("If android/ exists, files were written directly. Otherwise, copy them into android/app/src/main/res/ then rebuild.");
}
